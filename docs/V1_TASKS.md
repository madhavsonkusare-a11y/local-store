...
| Q04 | PARTIAL | The pinned Memos 0.30 API probe creates the first admin, signs in, writes a private memo and reads its exact content after restart and keep-data reinstall on the owned WSL engine. The proof state is removed afterward; script bytes now enter the evidence fingerprint. This is one meaningful app-task proof, not broad first-use coverage. Build representative content probes for the other release apps; for AI apps, reuse a maintained mock HTTP server or fixture framework and distinguish stub integration from real-provider compatibility. | Q02 |
