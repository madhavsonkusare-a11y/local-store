...
| Q01 | DONE | Evidence schema 2 fingerprints the exact source locator, adapter and revision; normalized plan; requested and resolved images; OS/architecture; selected engine; Compose version; first-use probe; and evidence level. Batch scheduling reruns changed inputs, retains current passes and optionally retries current failures. Qualification proof expires after 30 days, source review after 90 days and image observation after 30 days; missing, malformed or implausibly future dates refuse. Older JSON remains readable history without retaining stronger claims. Twenty-two focused tests and strict Clippy pass. | E02 |
| Q02 | PARTIAL | All-service count, running/health and successful-job checks run after install/restart/reinstall; uncertain cleanup inventory fails qualification. Evidence records time-to-first-answer, idle/peak memory, managed bind and named-volume storage, and immutable image virtual size. Named volumes require verified project/volume labels and local driver; missing, foreign, partial and unmeasurable volumes fail. Historical evidence without volume measurements is not reusable. [Real managed-engine Memos](evidence/memos-managed-resource-2026-09-23.json) and [n8n](evidence/n8n-managed-resource-2026-09-23.json) runs each passed 18 steps, three resource samples, restart/reinstall and exact cleanup. n8n's earlier 120-second restart failure did not recur on the isolated rerun; its named volume measured 5,689,024 bytes after install. Neither run proves a meaningful in-app task. Explicit per-service memory, CPU and PID ceilings render from the plan, but no release app has measured limits yet. Apply and verify limits, then repeat managed-engine proof across release apps. | Q01 |
| Q03 | PARTIAL | Two of 52 current offerings (Memos and n8n) have passed isolated managed-WSL lifecycle/resource runs with recorded engine identity and ownership-safe cleanup; neither has a meaningful app-task probe, so neither satisfies the full release verification gate. Requalify the remaining offerings sequentially against the managed engine and stronger checks, record failures/demotions, and never inherit Docker Desktop proof across the engine swap. | E04, Q02 |
