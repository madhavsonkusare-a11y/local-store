...
| E02 | PARTIAL | New installs persist local endpoint bindings; lifecycle, rollback, qualification and recovery use them, including post-deletion checks. [Real Memos context-change regression](evidence/engine-binding-memos-2026-09-13.json) passes. Explicit legacy adoption (`bind-engine`) verifies existing container ownership before persisting a local binding. [Real legacy-shaped Memos adoption](evidence/engine-adoption-memos-2026-09-14.json) passes restart/reinstall and cleanup under an invalid ambient context. Experimental schema-2 WSL binding, typed Compose/bind/seed projection, lifecycle routing, rollback and dual-path recovery have fixture coverage; real Compose parsing passes. Owned bootstrap, selection gating, managed qualification and real WSL proof remain open. Introduce engine selection through existing process/runtime seams. All install, doctor, qualification and recovery operations use the same selected engine/context. Existing Docker Desktop path still passes; fake alternate executable tests prove no hard-coded escape. | E01 |
| E03 | PARTIAL | Journal, collision preflight, rootfs verification, bounded import and post-import identity/component/daemon verification are implemented. Read-only recovery classifies retry, resume verification, ready or manual review and never treats an uncertain footprint as owned. Retry reuses the immutable reservation and re-verifies the rootfs. Unregistration can only be prepared from a Verified journal after the external and in-distro ownership tokens match; execution remains a separate explicit decision. Prerequisites/elevation, product wiring and clean-machine proof remain. | E02 |
