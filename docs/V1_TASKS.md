...
Updated September 19, 2026. **This is the only active release task ledger.**
...
| Q02 | PARTIAL | All-service count, running/health and successful-job checks run after install/restart/reinstall; uncertain cleanup inventory fails qualification. Evidence records time-to-first-answer, idle/peak memory, managed bind and named-volume storage, and immutable image virtual size. Named volumes require verified project/volume labels and local driver; missing, foreign, partial and unmeasurable volumes fail. Historical evidence without volume measurements is not reusable. [Real managed-engine Memos](evidence/memos-managed-resource-2026-09-23.json) and [n8n](evidence/n8n-managed-resource-2026-09-23.json) runs each passed 18 steps, three resource samples, restart/reinstall and exact cleanup. Memos now also proves an admin and private memo survive both transitions; n8n remains answers-only. n8n's earlier 120-second restart failure did not recur on the isolated rerun; its named volume measured 5,689,024 bytes after install. Explicit per-service memory, CPU and PID ceilings render from the plan, but no release app has measured limits yet. Apply and verify limits, then repeat managed-engine proof across release apps. | Q01 |
| Q03 | PARTIAL | Two of 52 current offerings (Memos and n8n) passed isolated managed-WSL lifecycle/resource runs with recorded engine identity and ownership-safe cleanup. Memos also passed a meaningful task; n8n has no task probe. Requalify the remaining offerings sequentially against the managed engine and stronger checks, record failures/demotions, and never inherit Docker Desktop proof across the engine swap. | E04, Q02 |
| Q04 | PARTIAL | The pinned Memos 0.30 API probe creates the first admin, signs in, writes a private memo and reads its exact content after restart and keep-data reinstall on the owned WSL engine. The proof state is removed afterward; script bytes now enter the evidence fingerprint. This is one meaningful app-task proof, not broad first-use coverage. The [at-scale qualification plan](plans/qualification-at-scale.md) groups 52 tasks into reusable fixture paths; Jev triage suggestions are all unreviewed and add zero proofs. Build shared content and controlled-HTTP drivers next; for AI apps, distinguish stub integration from real-provider compatibility. | Q02 |
...
| A02 | PARTIAL | An in-process policy now scopes grants by client/app/action, enforces expiry/revocation, requires an OS-random one-use approval for write/destructive actions, and records bounded audit metadata. The typed status invocation checks authorization before dispatch; tests cover cross-app denial, expiry, revocation, forged/reused approval and audit redaction. No externally authenticated identity, durable grant/audit store, broker-held secrets or agent transport exists yet. Grants are not a sandbox against same-user shell access. | A01 |
...
| C03 | PARTIAL | Release 50 selected offerings from today's 52 baseline. [Curated qualification ledger](../catalog/v1-qualified-apps.json) and `python scripts/check-v1-qualified.py` validate manifest/evidence/probe hashes, managed-Windows identity, freshness, lifecycle steps and task proof in CI; `--release-gate` fails until 50 qualify. Current evidence: two zero-answer managed-engine lifecycle passes (Memos and n8n), one setup-assisted meaningful-task proof (Memos first admin and private memo), so one of 50 has task-level proof. Every release offering still needs managed-engine lifecycle, setup and meaningful-task verification gates; separately list zero-input, setup-assisted and verified-task counts. Replace a demotion only through a reviewed promotion from the frozen planning roster. | Q03, Q04, C02 |
| C04 | PARTIAL | A launcher-only backend command parses strict HTTPS github.com repository URLs and resolves exact approved-offering matches. Unknown approved matches return pinned provenance only when the candidate queue contains an exact reviewed repository identity; otherwise they require review. Candidate definitions are never installed by this lookup. Tests cover approved, reviewed-but-unapproved, unknown and lookalike paths. Live commit-pinned metadata inspection, redirect handling, supported new candidates and install-preview UI remain. | A01, C01 |
