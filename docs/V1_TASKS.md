...
| C04 | PARTIAL | A launcher-only backend command parses strict HTTPS github.com repository URLs, resolves an exact source-repository match to an approved offering, reports ambiguity, and sends unknown repositories to review without executing repository content. Tests cover known paths, unknown repositories and lookalikes. Bounded commit-pinned metadata inspection, redirect handling, supported new candidates and install-preview UI remain. | A01, C01 |
