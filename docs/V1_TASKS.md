...
| E03 | PARTIAL | Bootstrap journal reserves immutable distro/data/payload ownership and permits only reserved → imported → verified transitions. Bounded inventory parsing and read-only preflight refuse name/data collisions. Rootfs is streamed through locked length/SHA-256 verification before reservation; only then is the fixed bounded import command returned. Execute/import identity verification, recovery and clean-machine proof remain. | E02 |
