...
| E03 | PARTIAL | Bootstrap journal reserves immutable distro/data/payload ownership and permits only reserved → imported → verified transitions. Bounded UTF-8/UTF-16LE-shaped distro inventory parsing and a read-only preflight now refuse name/data-directory collisions without mutation. Add authenticated payload staging, import/identity verification, recovery and clean-machine proof. | E02 |
