...
| Q02 | PARTIAL | All-service count, running/health and successful-job checks now run after install/restart/reinstall; uncertain cleanup inventory fails qualification. Evidence records time-to-first-answer plus idle and peak memory from project-scoped samples after install, restart and reinstall; evidence without all three samples cannot be reused. [Real Docker stopped-worker regression](evidence/service-health-regression-2026-09-13.json) passes. Finish image/disk measurements and explicit resource limits, then prove on the managed engine. | Q01 |
