...
| Q02 | PARTIAL | All-service count, running/health and successful-job checks now run after install/restart/reinstall; uncertain cleanup inventory fails qualification. Evidence records time-to-first-answer plus idle/peak memory and managed bind-storage bytes from project-scoped samples after install, restart and reinstall. On the selected managed WSL engine it checks each declared Compose volume's project/volume labels and local driver, then measures its disk bytes inside that distro; missing, foreign, partial and unmeasurable volumes fail the step. Historical evidence without volume measurements is not reusable. Each deduplicated immutable image's virtual size is also recorded; shared layers are not presented as a physical total. [Real Docker stopped-worker regression](evidence/service-health-regression-2026-09-13.json) passes. The plan can now render explicit per-service memory, CPU and PID ceilings while preserving existing definitions. Choose ceilings from managed-engine measurements, apply them to release apps, verify enforcement and collect real managed-engine proof; the volume sampler has fixture coverage only while Docker is unavailable. | Q01 |
