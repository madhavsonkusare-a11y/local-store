...
| E04 | PARTIAL | The launcher engine-status contract now distinguishes unchecked, responsive and unresponsive daemon state, probing only after verified WSL ownership. An internal repair routine starts Docker only inside that owned distro, skips an already responsive daemon and rechecks readiness. Fixture tests cover ownership refusal and exact command targeting. Owner-facing repair under an operation lock, coexistence, removal and sleep/wake/crash/restart proof remain; launcher close must not stop background apps. Engine and app-data removal are separate decisions. | E03 |
...
| Q02 | PARTIAL | All-service count, running/health and successful-job checks run after install/restart/reinstall; uncertain cleanup inventory fails qualification. Evidence records time-to-first-answer, idle/peak memory, managed bind and named-volume storage, and immutable image virtual size. Named volumes require verified project/volume labels and local driver; missing, foreign, partial and unmeasurable volumes fail. Historical evidence without volume measurements is not reusable. The [real managed-engine Memos run](evidence/memos-managed-resource-2026-09-23.json) passed 18 steps, three resource samples, restart/reinstall and exact cleanup. [n8n's managed run](evidence/n8n-managed-resource-2026-09-23.json) measured its real named volume after install, then failed because the app did not answer within 120 seconds after restart; cleanup left no containers/volumes. Explicit per-service memory, CPU and PID ceilings render from the plan, but no release app has measured limits yet. Diagnose n8n restart, apply and verify limits, then repeat managed-engine proof across release apps. | Q01 |
